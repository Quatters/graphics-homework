...
var depthLimit = document.querySelector('#depth').value || 7;
var hue = document.querySelector('#hue').value / 100 || 0.15;

document.querySelector('#depth').addEventListener('change', function (event) {
  depthLimit = event.target.value;
  draw();
});

document.querySelector('#hue').addEventListener('change', function (event) {
  hue = event.target.value / 100;
  draw();
});
